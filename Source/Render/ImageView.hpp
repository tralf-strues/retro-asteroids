...
class ImageView {
 public:
  ...
  constexpr ImageView(const ImageView<OtherPixelType>& other);

  constexpr std::span<PixelType> Row(uint32_t y);
  constexpr std::span<PixelType> Row(uint32_t y) const;

  constexpr PixelType& operator()(uint32_t x, uint32_t y);
  constexpr PixelType& operator()(uint32_t x, uint32_t y) const;

  constexpr const math::Vec2u& Offset() const;
  constexpr const math::Vec2u& Extent() const;

  constexpr std::span<PixelType> Data();
  constexpr std::span<PixelType> Data() const;
...
template <typename OtherPixelType>
  requires(sizeof(PixelType) == sizeof(OtherPixelType))
constexpr ImageView<PixelType>::ImageView(const ImageView<OtherPixelType>& other)
...
template <typename PixelType>
constexpr std::span<PixelType> ImageView<PixelType>::Row(uint32_t y) {
  return std::span(image_pixels_ + (region_offset_.y + y) * image_extent_.x + region_offset_.x, region_extent_.x);
}

template <typename PixelType>
constexpr std::span<PixelType> ImageView<PixelType>::Row(uint32_t y) const {
  return std::span(image_pixels_ + (region_offset_.y + y) * image_extent_.x + region_offset_.x, region_extent_.x);
}

template <typename PixelType>
constexpr PixelType& ImageView<PixelType>::operator()(uint32_t x, uint32_t y) {
  return Row(y)[x];
}

template <typename PixelType>
constexpr PixelType& ImageView<PixelType>::operator()(uint32_t x, uint32_t y) const {
  return Row(y)[x];
}

template <typename PixelType>
constexpr const math::Vec2u& ImageView<PixelType>::Offset() const {
  return region_offset_;
}

template <typename PixelType>
constexpr const math::Vec2u& ImageView<PixelType>::Extent() const {
  return region_extent_;
}

template <typename PixelType>
constexpr std::span<PixelType> ImageView<PixelType>::Data() {
...
template <typename PixelType>
constexpr std::span<PixelType> ImageView<PixelType>::Data() const {
  RA_ASSERT(region_offset_ == math::Vec2u(0U) && region_extent_ == image_extent_,
            "In order to access entire ImageView data, it must cover the entire underlying image");
